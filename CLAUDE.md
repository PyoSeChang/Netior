...
- **`packages/moc-core`** (`@moc/core`) — DB 로직 (connection, repositories, migrations). tsup (ESM+CJS). desktop-app과 moc-mcp가 공유.
- **`packages/moc-mcp`** (`@moc/mcp`) — MCP 서버. moc-core를 17개 도구로 래핑. stdio transport (Claude Code 등록 가능).
- **`packages/agent-server`** (`@moc/agent-server`) — Narre AI 에이전트. Claude Agent SDK + Express. SSE 스트리밍. 세션 관리. DB 직접 접근 없음 (moc-mcp 경유).
...
NarreChat.tsx         →IPC→ narre-ipc.ts      →HTTP→ Express :3100
  SSE stream events   ←IPC← (forward SSE)     ←SSE←  Claude Agent SDK query()
                                                         │
                                                    moc-mcp (stdio MCP 서버)
                                                         │
                                                    @moc/core → moc.db
```

**데이터 흐름:**
- agent-server는 DB에 직접 접근하지 않음. 모든 DB 작업은 moc-mcp 경유.
- 시스템 프롬프트용 메타데이터(아크타입, 릴레이션타입 등)는 desktop-app Main이 DB에서 조회 → `/chat` body로 전달.
- better-sqlite3 네이티브 모듈은 Electron(desktop-app)과 Node.js(moc-mcp)가 별도 프로세스로 사용. agent-server는 네이티브 모듈 없음.
