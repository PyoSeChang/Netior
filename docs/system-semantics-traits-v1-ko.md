...
이 문서는 Netior가 사용자 정의 도메인을 앱이 해석 가능한 의미 체계로 읽기 위해 사용하는 `system semantics`의 1차 기준안을 정의한다.

핵심 목표는 다음과 같다.

- 사용자가 자유롭게 아크타입 이름과 속성 라벨을 정해도 앱은 그 의미를 이해할 수 있어야 한다.
- 캘린더, 타임라인, 보드, 트리, 대시보드 같은 뷰는 도메인 이름이 아니라 안정적인 의미 슬롯을 읽어야 한다.
- 같은 아크타입에 여러 의미를 조합할 수 있어야 한다.

이 문서는 대분류와 trait 목록을 먼저 고정하고, 이후 DSL과 DB 스키마, 자동 생성 규칙의 기준으로 사용한다.
...
- `semantic trait`
  아크타입에 부착되는 조합 가능한 의미 단위다.
- `canonical slot`
  앱이 실제로 읽는 안정적인 의미 키다. 사용자는 이 슬롯에 대응하는 속성의 라벨을 자유롭게 바꿀 수 있다.

현재 기준에서:

- 아크타입은 `semantic traits`를 가진다.
- edge의 기존 `system_contract`는 개념적으로 같은 우산 아래에 있지만, 필드명 변경은 별도 마이그레이션으로 다룬다.
- `node_type`는 아크타입 의미가 아니라 네트워크 안에서 해당 노드가 수행하는 역할을 뜻하며, `semantic trait`와는 별개로 유지한다.

## 3. 설계 원칙

- trait는 뷰 이름이 아니라 의미 단위여야 한다.
- 하나의 아크타입은 여러 trait를 동시에 가질 수 있다.
- 앱은 사용자 라벨이 아니라 canonical slot을 읽는다.
- trait는 필요한 속성을 자동 생성하고 타입을 강제할 수 있어야 한다.
- trait는 UI 힌트가 아니라 런타임 의미 체계다.
...
v1에서는 trait 탐색과 선택 UX를 위해 다음 대분류를 사용한다.
...
아래 표는 v1에서 실제 설계 기준으로 삼는 상세 규칙이다. 여기서 `core slot`은 trait 부착 시 기본적으로 생성 또는 요구되는 슬롯이고, `optional slot`은 필요 시 추가되는 슬롯이다.

### 6.1 Time

| Trait | Core slot | Optional slot | 기본 타입 | 기본 해석 |
| --- | --- | --- | --- | --- |
...
| `taggable` | `tag_keys` | - | `string[]` | 다중 라벨 분류. filter chip, quick grouping, search facet에 사용한다. |
...
trait는 뷰와 직접 1:1로 대응하지 않지만, 앱은 대략 아래처럼 trait를 읽어 각 뷰를 활성화할 수 있다.
...
| `table/list` | 대부분 trait를 공통적으로 표시 가능 |

## 8. 권장 trait 조합

단일 trait보다 조합이 실제 도메인 모델에 더 가깝다.
...
- trait 부착 시 필요한 canonical slot을 자동 생성할 수 있어야 한다.
- 사용자는 slot에 연결된 속성의 라벨을 자유롭게 바꿀 수 있어야 한다.
- slot 타입은 trait가 강제할 수 있어야 한다.
- trait 충돌 시 canonical slot 타입이 다르면 부착을 막는다.
...
즉, v1에서는 `대분류`, `trait 목록`, 그리고 각 trait의 `core/optional slot`과 기본 해석 규칙까지 기준안으로 고정한다.
