...
- 사용자는 Netior의 network, node, edge, schema, model, field definition를 몰라도 된다.
...
- schema, field definition, model, meaning slot, ORM형 참조 설계
...
중요한 점은 `objects` 테이블이 독립적인 사용자 편집 대상이 아니라는 것이다. `concept`, `network`, `schema`, `relation_type`, `type_group`, `file`, `project`가 만들어질 때 object record가 함께 만들어지고, Narre는 이 object record를 조회해서 `network_nodes`에 배치한다.

현재 코드 기준으로 `Universe`와 `Ontology`는 일반 network가 아니라 시스템 네트워크다.

- `Universe`: 앱 전체 프로젝트 포털 네트워크. project object가 portal node로 배치된다.
- `Ontology`: 프로젝트의 타입/스키마 네트워크. type group, schema, relation type object가 자동 투영된다.
...
| NR09 | recurrence와 virtual occurrence | recurring concept, occurrence materialization, meaning slot 기반 반복 구조를 이해하는가 |
...
| NR17 | schema field definition CRUD | field type, required, default, option, order, ref schema, meaning slot을 다루는가 |
| NR18 | model와 meaning slot | model를 고르고 model-generated slot field를 구성하는가 |
...
- Ontology는 프로젝트의 타입/스키마 네트워크이며, type group/schema/relation type object가 자동 투영된다.
...
- group/list/grid 정렬 기준을 meaning slot 또는 property field로 잡을 수 있음을 이해한다.
...
- `meaning_slot` validation과 allowed field type을 고려한다.
- 사용자가 model나 field를 몰라도 Narre가 적절한 property 값을 넣어야 한다.
...
- `recurrence_rule`, `recurrence_until`, `recurrence_count`, `start_at`, `end_at` 같은 meaning slot을 이해한다.
...
- `relation_type_id`, `relation_meaning`, `description`을 구분한다.
...
- `name`, `description`, `icon`, `color`, `node_shape`, `file_template`, `semantic_models`, `group_id`를 다룬다.
...
- `field_type`, `required`, `default_value`, `options`, `ref_schema_id`, `meaning_slot`, `slot_binding_locked`, `generated_by_model`를 구분한다.
- field가 concept property 입력과 어떻게 연결되는지 이해한다.

### NR18. model와 meaning slot
...
- 사용자가 model를 모른다는 전제로 domain에서 model를 추론한다.
- 예: 일정/마감은 `temporal`, `dueable`; 상태 흐름은 `statusful`; 계층 구조는 `hierarchical`; 자료 출처는 `sourceable`.
- model를 붙이면 필요한 meaning slot field가 생기거나 유지되어야 함을 이해한다.
- model-generated field와 사용자가 만든 detached slot field를 구분한다.
...
- 이후 schema, model, field definition, relation type, network split, starter concept/node/edge 생성 계획을 제안한다.
...
- **Network object CRUD**: concept, 일반 network, schema, relation type, type group, file object를 만들고 수정하고 삭제하고 node로 배치한다.
- **Ontology/schema 추론**: 사용자가 모르는 model, field definition, ORM형 관계, relation type을 Narre가 도메인에서 추론한다.
...
- model와 meaning slot 사용
- schema_ref 또는 relation field 기반 ORM형 관계
- relation type과 edge를 통한 그래프 관계
- Ontology network에 투영되는 type group/schema/relation type 구조
