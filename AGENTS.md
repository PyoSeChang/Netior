...
pnpm --filter @netior/agent-server build  # agent-server만 빌드 (tsup)
...
- **`packages/netior-core`** (`@netior/core`) — DB 로직 (connection, repositories, migrations). tsup (ESM+CJS). desktop-app과 agent-server가 공유.
- **`packages/netior-mcp`** (`@netior/mcp`) — MCP 서버. moc-core를 17개 도구로 래핑. stdio transport (Codex 등록 가능).
- **`packages/agent-server`** (`@netior/agent-server`) — Narre AI 에이전트. Anthropic SDK + Express. SSE 스트리밍. 세션 관리.
...
- **Build order**: `@netior/shared` → `@netior/core` → `@netior/mcp`, `@netior/agent-server`, `@netior/desktop-app` (turbo `dependsOn: ["^build"]`).
- **DB 동시 접근**: WAL 모드 + `busy_timeout(5000)`. desktop-app은 moc-core를 in-process로, netior-mcp/agent-server는 별도 프로세스로 같은 DB 파일 접근.
